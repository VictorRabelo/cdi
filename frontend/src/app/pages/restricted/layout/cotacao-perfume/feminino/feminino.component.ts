...
export class FemininoComponent extends ControllerBase {
  ...
  getAll(){
    this.sub.sink = this.produtoService.getFeminino().subscribe(
      (res: any) => {
        this.loading = false;
        this.perfumes = res;
  ...
  atualizar(){
    ...
    this.produtoService.storeDolarFeminino(store).subscribe(
      (res: any) => {
        this.loading = true;
        this.getAll();
      },
